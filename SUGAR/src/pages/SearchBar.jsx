...
//     { name: 'CONCEALER', path: '/skin' },
//     { name: 'BULLET LIPSTICKS', path: '/lip ' },
//     { name: 'LIPSTICK', path: '/new' }
//   ];
  
//   const recommendedProducts = [
//     { id: 1, name: 'ACE OF FACE DEWY FOUNDATION',path:'/FP', price: 799.00, image: 'https://cdn.shopify.com/s/files/1/0906/2558/files/Ace-of-Face-Dewy-Foundation-2_b4f5555e_large.jpg?v=1741431621' },
//     { id: 2, name: 'TIP TAC TOE NAIL LACQUER - CLASSIC DUO',path:'nail', price: 349.00, originalPrice: 398.00, image: 'https://cdn.shopify.com/s/files/1/0906/2558/files/Tip-Tac-Toe-Nail-Lacquer-Classic-Duo_large.jpg?v=1736172151', discount: '12%' },
//     { id: 3, name: 'CRIMSON KISSES LIP SET',path:'/new', price: 899.00, originalPrice: 1197.00, image: 'https://cdn.shopify.com/s/files/1/0906/2558/files/Crimson-Kisses-Lip-Set_large.jpg?v=1736172096', discount: '25%' },
//     { id: 4, name: 'MATTE ATTACK TRANSFERPROOF LIPSTICK',path:'/lip', price: 999.00, originalPrice: 1398.00, image: 'https://cdn.shopify.com/s/files/1/0906/2558/files/Matte-Attack-Transferproof-Lipstick-Seal-The-Show-Lip-Primer_large.jpg?v=1736171904', discount: '29%' },
//     { id: 5, name: 'MOUSSE MUSE LIP CREAM & FOUNDATION STICK',path:'/FP', price: 999.00, originalPrice: 1158.00, image: 'https://cdn.shopify.com/s/files/1/0906/2558/files/Mousse-Muse-Lip-Cream-Foundation-Stick-Combo_large.jpg?v=1736171873', discount: '14%', tag: 'COMBO' }
//   ];
...
const SearchBar = () => {
  ...
  const filteredProducts = recommendedProducts.filter(product =>
    product.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    product.path.toLowerCase().includes(searchTerm.toLowerCase()) // Search in path
  );
            ...
            <div className="bg-white text-black w-full max-w-4xl mt-12 rounded-lg shadow-lg overflow-y-auto max-h-[80vh]">
              ...
              <div className="p-4 flex justify-between border-b">
                ...
                <input 
                  ...
                  className="w-full p-2 pl-10 border rounded-md bg-black text-white" 
                />
                <button onClick={closeSearch} className="text-gray-600 hover:cursor-pointer">❌</button>
              </div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {popularChoices.map((choice, index) => (
                    <button 
                      ...
                      className="px-4 py-2 border rounded text-sm text-white bg-black cursor-pointer hover:bg-gray-800"
                    >
                      {choice.name} ➜
                    </button>
                  ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map(product => (
                      <div 
                        key={product.id} 
                        className="relative cursor-pointer hover:shadow-lg transition-shadow p-2 rounded-lg"
                        onClick={() => navigate(product.path)}
                      >
                        {product.discount && <div className="absolute top-2 left-2 bg-white text-black text-xs px-2 py-1">SAVE {product.discount}</div>}
                        {product.tag && <div className="absolute top-2 right-2 bg-white text-black text-xs px-2 py-1">{product.tag}</div>}
                        <img src={product.image} alt={product.name} className="w-full h-44 object-cover rounded-md" />
                        <h3 className="text-sm font-medium text-black mt-2">{product.name}</h3>
